...
export const ModalLink = ({ isOpen, onClose, title, userId }: ModalProps) => {
  ...
  return (
    <div
      data-modal="form"
      className="fixed inset-0 bg-black bg-opacity-60 overflow-y-auto h-full flex justify-center items-center transition-opacity duration-300"
    >
      <div className="bg-white w-[700px] rounded-lg p-6 mx-5 md:mx-0">
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            onClick={onClose}
          >
            <Icons.Close className="icon w-5"/>
          </button>
        </div>

        <form
          className="mt-5 grid grid-cols-4 grid-rows-2 xl:grid-rows-1 gap-7"
          onSubmit={handleOutsideClick}
        >
          ...
          <button
            type="submit"
            className="text-white col-span-4 xl:col-span-1 rounded-2xl bg-blue-500 hover:bg-blue-700 transition ease-in"
          >
            Add
          </button>
        </form>
